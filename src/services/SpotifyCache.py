...
class Album(BaseModel):
  ...
  class Config:
    use_enum_values = True  # This will make the Enum serializable as its value (string)
...
class SpotifyCache:
  ...
  def get_or_fetch_top_tracks(self, artist_id: str) -> List[Track]:
    key = f"artist:top_tracks:{artist_id}"
    data = self.r.get(key)
    if data:
      return self._deserialize(Track, data)

    tracks = self.sp.artist_top_tracks(artist_id)['tracks']
    result = [self._convert_to_track(t) for t in tracks]

    self.r.setex(key, TOP_TRACKS_CACHE_TIME, self._serialize(result))
    return result

  def get_or_fetch_releases(self, artist_id: str) -> List[Album]:
    key = f"artist:albums:{artist_id}"
    data = self.r.get(key)
    if data:
      return self._deserialize(Album, data)
    ...
    self.r.setex(key, ALBUMS_CACHE_TIME, self._serialize(result))
    return result

  def get_or_fetch_album_tracks(self, album_id: str) -> List[Track]:
    ...
    result = [self._convert_to_track(t, album_id) for t in tracks]

    self.r.setex(key, ALBUM_TRACKS_CACHE_TIME, self._serialize(result))
    return result


dings = SpotifyCache().get_or_fetch_releases("6LwKCnpeCVffopaYO3pafG")
print([i.url for d in dings for i in d.images ])